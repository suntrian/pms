...
public class Diff<T> implements Serializable {
    ...
    private boolean regardNullAsEmpty = true;
    //保留不变的数据
    ...
    @SafeVarargs
    public static <T> Boolean isEqual(T former, T later, boolean ignoreNull, boolean ignoreEmpty, boolean caseInSensitive, boolean regardNullAsEmpty, Function<T, ? extends Comparable>... propGetters) {
        for (Function<T, ? extends Comparable> getter : propGetters) {
            Comparable latterProp = getter.apply(later);
            Comparable formerProp = getter.apply(former);
            boolean equal = isEqual(formerProp, latterProp, ignoreNull, ignoreEmpty, caseInSensitive, regardNullAsEmpty);
            if (!equal) {
                return false;
            }
        }
    ...
    public static boolean isEqual(Object former, Object later, boolean ignoreNull, boolean ignoreEmpty, boolean caseInSensitive, boolean regardNullAsEmtpy) {
        ...
        if (former == null) {
            return regardNullAsEmtpy ? (later == null || "".equals(later)) : later == null;
        } else if (regardNullAsEmtpy && "".equals(former)) {
            return later == null || "".equals(later);
        }
    ...
    public <D extends Diff<T>> D regardNullAsEmpty(boolean regardNullAsEmpty) {
        this.regardNullAsEmpty = regardNullAsEmpty;
        return (D) this;
    }
    ...
    public <C extends Collection<T>, D extends Diff<T>> D diff(C formerCollection,
        ...
        if (formerCollection == null || laterCollection == null) {
            throw new IllegalArgumentException("Null Collection");
        }
        if (formerCollection.size() == 0) {
            this.created.addAll(laterCollection);
            return (D) this;
        }
        if (laterCollection.size() == 0) {
            this.deleted.addAll(formerCollection);
            return (D) this;
        }
     ...
     */
    @SafeVarargs
    public final <C extends Collection<T>, M extends Comparable<M>, D extends Diff<T>> D diff(final C formerCollection, final C laterCollection, final Function<T, M> identity, final Function<T, ? extends Comparable>... keyProperties) {
        Comparator comparator = Comparator.comparing(identity);
        return (D) diff(formerCollection, laterCollection, comparator, comparator, comparator, (T former, T later) -> isEqual(former, later, this.ignoreNull, this.ignoreEmpty, this.caseInSensitive, this.regardNullAsEmpty, keyProperties));
    }

    @SafeVarargs
    public final <C extends Collection<T>, M extends Comparable<M>, D extends Diff<T>> D diff(final C formerCollection, final C laterCollection, final Comparator<T> comparator, final Function<T, ? extends Comparable>... keyProperties) {
        return diff(formerCollection, laterCollection, comparator, comparator, comparator, (T former, T later) -> isEqual(former, later, this.ignoreNull, this.ignoreEmpty, this.caseInSensitive, this.regardNullAsEmpty, keyProperties));
    }

    @SafeVarargs
    public final <C extends Collection<T>, M extends Comparable<M>, D extends Diff<T>> D diff(final C formerCollection, final Function<T, M> formerIdentityGetter, final C laterCollection, final Function<T, M> laterIdentityGetter, final Function<T, ? extends Comparable>... keyProperties) {
        return diff(formerCollection, laterCollection, Comparator.comparing(formerIdentityGetter), Comparator.comparing(laterIdentityGetter), (T former, T later) -> formerIdentityGetter.apply(former).compareTo(laterIdentityGetter.apply(later)), (T former, T later) -> isEqual(former, later, this.ignoreNull, this.ignoreEmpty, this.caseInSensitive, this.regardNullAsEmpty, keyProperties));
    }
    ...
    public static class StringDiff extends Diff<String> {
        ...
        public <C extends Collection<String>> StringDiff diff(final C former, final C later) {
    ...
    public static class IntDiff extends Diff<Integer> {
        ...
        public <C extends Collection<Integer>> IntDiff diff(final C former, final C later) {
            return diff(former, later, Comparator.naturalOrder());
        }
    }
}
