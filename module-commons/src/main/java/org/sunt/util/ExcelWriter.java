...
public final class ExcelWriter {
  ...
  private CellStyle setCellStyleFont(@Nonnull CellStyle style,
    ...
    for (BorderPosition position : positions) {
      switch (position) {
        ...
        case TOP:
          ...
          ;
          if (color != null) {
            style.setTopBorderColor(color.shortValue());
          }
          ;
          if (!all) break;
        case BOTTOM:
          ...
          if (color != null) {
            style.setBottomBorderColor(color.shortValue());
          }
          if (!all) break;
        case LEFT:
          if (borderStyle != null) {
          ...
          ;
          if (color != null) {
            style.setLeftBorderColor(color.shortValue());
          }
          ;
          if (!all) break;
        case RIGHT:
          ...
          ;
          if (color != null) {
            style.setRightBorderColor(color.shortValue());
          }
          ;
          if (!all) break;
      }
    ...
    if (this.customStyle != null && !this.customStyle.isEmpty()) {
      ...
      try {
        for (ListIterator<Map<CellRangeAddress, CellStyle>> iterator = this.customStyle.listIterator(); iterator.hasNext(); ) {
          ...
          List<MutablePair<Integer, Integer>> xRangeList = cellStyleMap.keySet().stream().map(x -> MutablePair.of(x.getFirstColumn(), x.getLastColumn())).sorted(Comparator.comparing(Pair::getLeft)).collect(Collectors.toList());
          handleRangeList(xRangeList);

          List<MutablePair<Integer, Integer>> yRangeList = cellStyleMap.keySet().stream().map(x -> MutablePair.of(x.getFirstRow(), x.getLastRow())).sorted(Comparator.comparing(Pair::getLeft)).collect(Collectors.toList());
  ...
  private void handleRangeList(List<MutablePair<Integer, Integer>> rangeList) {
    MutablePair<Integer, Integer> formerOne = MutablePair.of(Integer.MIN_VALUE, Integer.MIN_VALUE);
    for (Iterator<MutablePair<Integer, Integer>> iter = rangeList.iterator(); iter.hasNext(); ) {
      MutablePair<Integer, Integer> pair = iter.next();
